...
let append3Div = () =>{
    let DivWrapper = document.querySelector(".week__wrapper");
    let threeDivWrapper = document.createElement("div");
    threeDivWrapper.setAttribute("class",weekdays);
    threeDivWrapper.classList.add("weekdays");
    let divTitle = document.createElement("div");
    divTitle.classList.add("title");
    let divTitle_h1 = document.createElement("h1");
    divTitle_h1.textContent=weekdays;
    divTitle.appendChild(divTitle_h1);
    DivWrapper.appendChild(divTitle);
    DivWrapper.appendChild(threeDivWrapper);

}
let createDiv = (weekday,start,end,title,cover) =>{
    let DivWrapper = document.querySelector(".week__wrapper");

    // div
    let Div = document.createElement("div");
    Div.setAttribute("class","week-wrapper__card");
    ...
    while (start<end){
        ...
        button.addEventListener("click",()=>{
            localStorage.setItem(weekday,start);
            button.classList.toggle("checked");
        })
    
        button.textContent = start+'-'+(start+2)+" სთ";
    ...
    photo.setAttribute("src",cover);
    photo.setAttribute("class","cover");

    Div.appendChild(titleName);
    Div.appendChild(photo);
    Div.appendChild(times);
    let thisWeekdayDiv = document.querySelector("."+weekday);
    thisWeekdayDiv.appendChild(Div);

}
for (weekdays in data[0]){
    console.log(data[0][weekdays]);
    append3Div();
    
    for (i in data[0][weekdays]){       
        console.log(i);
        console.log(weekdays)
        createDiv(weekdays,data[0][weekdays][i].start,data[0][weekdays][i].end,data[0][weekdays][i].title,data[0][weekdays][i].cover);       
    }
...
const newNavMenu = document.querySelectorAll(".burgermenu");
